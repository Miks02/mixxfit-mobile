...
const DashboardScreen = () => {
  ...
  if (isError) {
    return (
      <Text className="text-3xl text-red-700 font-semibold">
        An unspecified error has occurred. Please try again later...
      </Text>
    );
  }

  const getWorkoutStreakMessage = (streak: number) => {
    if (!streak || streak === 0) return `Not on a streak`;
    if (streak === 1) return `${streak} Day - Keep it going!`;
    if (streak >= 2 && streak < 5) return `${streak} Days - Well done!`;
    if (streak >= 5) return `${streak} Days - Outstanding!`;
